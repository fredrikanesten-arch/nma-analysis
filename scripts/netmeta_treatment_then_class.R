...
# Fallback class mapping used only for treatments missing in trt_map.
...
if (!is.na(reference_treatment_arg)) {
  ...
  placebo_candidates <- trt_map_dedup %>%
...
treatment_class_lookup <- trt_map_dedup %>%
...
saveRDS(
  list(
    input_data = dat,
    treatment_class_map = trt_map_dedup,
    dat_audit = dat_audit,
